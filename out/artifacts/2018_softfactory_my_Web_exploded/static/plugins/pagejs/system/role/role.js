function initTable() {
    ...
    $("#roleTable").bootstrapTable({
        ...
        url: "role/select", //获取数据的Servlet地址
        dataType: "json",
...
function actionFormatter(value, row, index) {
    ...
    if ($.canGrant) {
        str += ("<a title='授权' href='javascript:;' onclick=role_grant('授权','role/role_grant/"
            + row.id + "','300','350')  style='text-decoration:none'><i class='Hui-iconfont'>&#xe62d;</i></a>");
    }
...
function datadel() {
    var row = $.map($("#roleTable").bootstrapTable('getSelections'),
        function (row) {
            return row;
        });
